...
struct EntryEditingTests {
    ...
    @Test("Saving people on another entry preserves earlier associations")
    func peopleSavePreservesOtherEntries() throws {
        let context = try makeContext()
        let person = Person(name: "Emma")
        let firstEntry = LogEntry(kind: .placeVisit, needsReview: false)
        firstEntry.placeVisitDetails = PlaceVisitDetails()
        let secondEntry = LogEntry(kind: .transit, needsReview: false)
        secondEntry.transitDetails = TransitDetails(type: "Bus")
        context.insert(person)
        context.insert(firstEntry)
        context.insert(secondEntry)
        try context.save()

        let firstSession = EntryDetailEditSession(entry: firstEntry)
        firstSession.selectedPeopleIDs = [person.id]
        try EntryDetailEditingService.savePeople(
            entry: firstEntry,
        ...
        let secondSession = EntryDetailEditSession(entry: secondEntry)
        secondSession.selectedPeopleIDs = [person.id]
        try EntryDetailEditingService.savePeople(
            entry: secondEntry,
        ...
        #expect(firstEntry.people.map(\.id) == [person.id])
        #expect(secondEntry.people.map(\.id) == [person.id])

        let reloadedContext = ModelContext(context.container)
        let reloadedEntries = try reloadedContext.fetch(
            FetchDescriptor<LogEntry>()
        )
        #expect(reloadedEntries.count == 2)
        #expect(
            reloadedEntries.allSatisfy {
                $0.people.map(\.id) == [person.id]
            }
        )
    }
